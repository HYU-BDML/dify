...
- ✅ (해소) PROV-01 소스는 배포 이미지(1.16.1-boram3)에만 있고 저장소에 미푸시 상태였다 —
  이미지 레이어에서 `boram_provision.py`·`boram_copy.py` 를 추출해 이 브랜치에 복원했다
  (`controllers/console/` 등록 + `BORAM_PROVISION_ENABLED` config 포함). 단, 이미지의
  베이스 커밋이 이 브랜치 베이스와 달라(업스트림 스큐) **이미지 재빌드 전 통합 테스트 필수**.
